...
function checkAndUpgradeHook(): void {
  // Check if we're running in a shell environment
  if (process.env.COMMIT_MSG_HOOK_PATH && process.env.COMMIT_MSG_HOOK_VERSION) {
    const hookPath = process.env.COMMIT_MSG_HOOK_PATH;
    const hookVersion = parseInt(process.env.COMMIT_MSG_HOOK_VERSION, 10);
